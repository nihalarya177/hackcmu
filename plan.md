...
- ~~Hosted migrations not applied.~~ **Done.** Applied to project `xuzoeolsoqdbzrqbgeub` on Kartik's confirmation: 16 tables, RLS enabled on all 16, `trip_api` and `trip_worker` group roles created, and exactly `trip` and `message` published to `supabase_realtime`. The API answers `/health/ready` against it.
